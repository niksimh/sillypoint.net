...
test("Make bad move that is not an int as p1", () => {
  ...
  let rightResult: PlayerMoveBadInputResult = {
    decision: "badInput"
  };

  expect(playerMoveLogic("pId1", game, "badInput")).toEqual(rightResult);
})
...
test("Make bad move as p2", () => {
  ...
  expect(playerMoveLogic("pId2", game, "badInput")).toEqual(rightResult);
})
